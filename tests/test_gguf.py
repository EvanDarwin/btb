...
def test_the_cpu_kquant_kernels_match_the_gguf_dequantization() -> None:
    ...
    verify pass's contract); skipped without the native library or its k-quant kernels."""
    ...
    need_native()
    if Native.gemv_q4k is None or Native.gemv_iq4xs is None:
        pytest.skip("the native library predates the CPU packed kernels")
    rows, cols = 40, 512
    cases = [
...
def test_the_cpu_lattice_kernels_match_the_gguf_dequantization() -> None:
    ...
    for the ksigns types) come from the gguf package, as the MLX path reads them. Skipped without the native
    library or its lattice kernels."""
    ...
    need_native()
    if Native.gemv_iq3xxs is None or Native.gemv_iq1m is None:
        pytest.skip("the native library predates the CPU lattice kernels")
...
def test_a_gguf_multiplies_its_blocks_as_stored_on_the_cpu() -> None:
    ...
    tokens match. Skipped without the native library or the cached file."""
    from btb.engine.native import Native

    need_native()
    if Native.gemv_q4k is None:
        pytest.skip("the native library predates the CPU packed kernels")
    import btb
    ...
    logits, toks = {}, {}
    for packed in (1, 0):
        with btb.load(path, device="cpu", gguf_packed=packed, log=None) as sm:
            if packed:
                bound = any(
