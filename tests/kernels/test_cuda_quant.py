...
DECODERS: dict[Quant, tuple[Decoder, ScaleFields]] = {
    ...
    Quant.Q4_K: (_decode_q4k, (0, 2)),
    Quant.Q5_K: (_decode_q5k, (0, 2)),
    Quant.Q6_K: (_decode_q6k, (208,)),
}
_Q4K = QUANTS[Quant.Q4_K].block_bytes


def _blocks(name: Quant, n: int, seed: int) -> bytes:
...
def test_the_card_decoder_matches_gguf_bit_for_bit(name: Quant) -> None:
    nb = QUANTS[name].block_bytes
...
def test_the_q4k_matvec_is_the_dequantized_product_and_batch_invariant() -> None:
    R, nsb = 12, 2
    C = nsb * 256
    W = b"".join(_blocks(Quant.Q4_K, nsb, 100 + r) for r in range(R))
